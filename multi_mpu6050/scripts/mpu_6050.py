...
class MPU6050:
    ...
    # @param [in] address_mpu_master - MPU-6050 I2C address (default:MPU6050_ADDRESS_68[0x68]).
    # @param [in] address_mpu_slave - MPU-6050 I2C slave address (default:[None]).
    ...
    def __init__(self, 
        address_mpu_master=MPU6050_ADDRESS_68, 
        address_mpu_slave=None, 
        bus=1, 
        gfs=GFS_2000, 
        afs=AFS_16G, 
    ...
    def reset(self, retry=3):
        ...
        try:
            ...
            if self.hasSlave():
                self.resetMPU6050Slave()

            self.resetMPU6050Master()
    ...
    # @param [in] self - The object pointer.
    def resetMPU6050Slave(self):
        self.setSlaveToWrite()
        
    # Reset all master registers to default.
    # @param [in] self - The object pointer.
    def resetMPU6050Master(self):
    ...
    #  @param [in] self - The object pointer.
    #  @retval [[timestamp], [accMaster], [gyroMaster], [accSlave], [gyroSlave], [tempMaster], [tempSlave] ] - all sensors data.   
    def getAllData(self): 
        ...
        try:
            ...
            return [timestamp] + accMaster + gyroMaster + accSlave + gyroSlave + [tempMaster] + [tempSlave]
    
        except OSError:
            return [timestamp] + self.getDataError() + self.getDataError() + self.getDataError() + self.getDataError() + [0, 0]        
    ...
    #  @param [in] self - The object pointer.
    #  @retval [[timestamp], [addresses], [fullScale], [resolution], [gbias], [gbias_slave], [abias], [abias_slave] ] - all sensors settings.   
    def getAllSettings(self):
        ...
        data = [
            ...
            self.gres,
            self.ares
        ] + self.gbias + self.gbias_slave + self.abias + self.abias_slave
